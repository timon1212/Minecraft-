document.addEventListener('DOMContentLoaded', () => {
  ...
  // ✅ Your OpenAI API Key
  const OPENAI_API_KEY = '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';
  ...
  function generateModelJSON(name, type) {
    if (type === 'crop') {
      return { parent: "block/cross", textures: { cross: `minecraft:blocks/${name}` } };
    } else {
      return { parent: "item/generated", textures: { layer0: `minecraft:items/${name}` } };
    }
  }
  ...
  generateBtn.addEventListener('click', async () => {
    ...
    const img = await generateTexture(prompt, type, style);
    if (!img) return;
    currentImage = img;
    drawTexture(img);
    createItemMesh(type, img);
    addItemToQueue(name, type, img);
  });

  // --- Download All Button ---
  downloadAllBtn.addEventListener('click', async () => {
    if (itemQueue.length === 0) return alert("No items in queue!");
